...
class System
	...
	def self.checkConf(conf=@gvitdConfFile)
		...
		if(@ini["OS"]["pythonExecPath"]) then
			@pythonExecPath = @ini["OS"]["pythonExecPath"]
		end
	...
	def self.checkEnv
		...
		#Pythonのパスが通っているか
		if(!File.exist?(@pythonExecPath)) then
			puts "Error[env]: Python not found.(#{@pythonExecPath})"
			exit
		end

		#Pythonのバージョンが2.7.xか
		s,e = Open3.capture3("#{@pythonExecPath} -V")
		pyVer = e.split(" ")[1]
